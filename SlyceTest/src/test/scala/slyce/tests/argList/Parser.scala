...
object Parser extends arch.Parser[String, List[String], Data.NonTerminal.ArgList] {
  ...
  private val grammar: Builder[Token, NonTerminal, NonTerminal.ArgList]#StateMachine =
    Builder.builder[Token, NonTerminal, NonTerminal.ArgList].build { builder =>
      ...
      lazy val s3: builder.State =
        builder.State(
          ...
          spontaneouslyGenerates = Nil,
          finalReturnF = Some {
            case elem(-\/(Token.EOF)) :: elem(\/-(rawTree: NonTerminal.ArgList)) :: Nil =>
              rawTree
          },
        )
          ...
          returnFs = List(
            {
              case elem(\/-(_4: NonTerminal.`Optional!`)) :: elem(-\/(_3: Token.__.`)`)) :: elem(\/-(_2: NonTerminal.AnonList1)) :: stateElem(state, -\/(_1: Token.__.`(`)) :: stackT =>
                (
                  state,
                  NonTerminal.ArgList._1(_1, _2, _3, _4),
                  stackT,
                )
            },
          ),
